...
function archive(
  id = "test.example",
  events: string[] = ["onCommand:test.hello"],
): ExtensionArchive {
  return validateArchive({
    ...
    manifest: {
      ...
      name: "Test",
      version: "1.0.0",
      engines: { l8db: ">=0.1.0 <1.0.0" },
      main: "dist/extension.js",
      activationEvents: events,
      permissions: ["database:read", "filesystem:extension-storage"],
      contributes: {
        commands: [{ id: "test.hello", title: "Hello" }],
        configuration: { "test.enabled": { type: "boolean", default: true } },
      },
    },
...
class MemoryStorage implements ExtensionStorage {
  ...
  async update(
    ...
    Object.assign(this.entries.get(id)!, { enabled, grants, configuration });
  }
  async get(id: string, key: string) {
    return this.values.get(`${id}:${key}`) ?? null;
  }
  async set(id: string, key: string, value: Json) {
    this.values.set(`${id}:${key}`, value);
  }
  async secretGet(id: string, key: string) {
    return this.secrets.get(`${id}:${key}`) ?? null;
  }
  async secretSet(id: string, key: string, value: string) {
    this.secrets.set(`${id}:${key}`, value);
  }
  async secretDelete(id: string, key: string) {
    this.secrets.delete(`${id}:${key}`);
  }
}
class TestRuntime implements ExtensionRuntime {
  ...
  async load(extension: ExtensionDescriptor, rpc: RpcHandler) {
    ...
    this.loads.push(id);
    this.rpc.set(id, rpc);
    this.descriptors.set(id, extension);
  }
  async activate(id: string) {
    this.activations.push(id);
    if (id === this.fail) throw new Error("broken extension");
    for (const command of this.descriptors.get(id)!.archive.manifest.contributes?.commands ?? [])
      await this.rpc.get(id)!("commands.register", [command.id]);
  }
  async deactivate(id: string) {
    this.deactivated.push(id);
    await this.rpc.get(id)!("logger", ["info", "deactivated"]);
  }
  async unload(id: string) {
    this.unloaded.push(id);
    this.rpc.delete(id);
  }
  async execute(id: string, _command: string, payload?: Json) {
    await this.rpc.get(id)!("notifications.show", ["info", "Hello", []]);
    return payload ?? "ok";
  }
...
function fakeCore() {
  ...
  const core: CoreServices = {
    ...
    showOpenDirectoryDialog: async () => "/tmp/project",
    showSaveDialog: async () => "/tmp/saved.txt",
    listDirectory: async (path, includeHidden) => ({
      path,
      entries: [...(directories.get(path) ?? [])].filter((entry) => includeHidden || !entry.hidden),
    }),
    watchPath: async (path, onChange) => {
    ...
    prompt: (async (request: unknown) => {
      ...
      return undefined;
    }) as CoreServices["prompt"],
  };
  return { core, notifications, clipboard, files, processes, prompts, directories, watchers };
}
function setup() {
...
describe("extension manifest", () => {
  test("normalizes main and API version", () => {
    const value = archive();
    value.manifest.main = "./dist/extension.js";
    expect(validateManifest(value.manifest).main).toBe("dist/extension.js");
    expect(validateManifest(value.manifest).engines.api).toBe("^1.0.0");
  });
  test("rejects malformed manifests", () => {
    for (const patch of [
      ...
      { main: "dist/NUL.js" },
      { permissions: ["root"] },
      { activationEvents: ["onCommand:missing"] },
      { engines: { l8db: "oops" } },
      { dependencies: { "test.example": "*" } },
      { contributes: { configuration: { x: { type: "boolean", default: "true" } } } },
    ])
      expect(() => validateManifest({ ...archive().manifest, ...patch })).toThrow();
  });
  test("rejects path traversal and absent compiled code", () => {
    const value = archive();
    value.files["../bad"] = "bad";
    expect(() => validateArchive(value)).toThrow();
  ...
  test("checks semver boundaries and prereleases", () => {
    ...
    for (const version of ["1.0.0", "0.0.9", "0.2.0-beta.1"])
      expect(() => assertCompatible(manifest, version)).toThrow(ExtensionError);
  });
});
describe("extension lifecycle integration", () => {
  ...
  test("discovery validates stored packages and isolates incompatibility", async () => {
    ...
    const bad = other("test.bad");
    bad.manifest.engines.l8db = ">=99.0.0";
    await storage.install(bad);
  ...
  test("startup and database activation events activate only enabled matching extensions", async () => {
    ...
    const database = other("test.database");
    database.manifest.activationEvents = ["onDatabaseOpen"];
  ...
  test("permission checks happen at host boundary and disposal removes event subscriptions", async () => {
    ...
    await expect(manager.enableExtension("test.example", ["network"])).rejects.toThrow(
      ExtensionError,
    );
    await manager.enableExtension("test.example", [
      "database:read",
    ...
    await allowed("storage.set", ["key", { value: 1 }]);
    expect(await allowed("storage.get", ["key"])).toEqual({ value: 1 });
  ...
  test("failure isolation and restart", async () => {
    const { manager, runtime } = setup();
    await manager.installExtension(archive());
    await manager.installExtension(other("test.good"));
    ...
    runtime.fail = "";
    await manager.reloadExtension("test.example");
    expect(await manager.executeCommand("test.hello")).toBe("ok");
  });
  test("duplicate contributions roll back installation", async () => {
    const { manager, storage } = setup();
    await manager.installExtension(archive());
    await expect(manager.installExtension(archive("test.duplicate"))).rejects.toThrow(
      ExtensionError,
    );
    expect(await storage.list()).toHaveLength(1);
    expect(manager.commands.owner("test.hello")).toBe("test.example");
  });
  test("configuration validates writes and isolates namespaces", async () => {
    const { manager } = setup();
    await manager.installExtension(archive());
    expect(manager.configuration.get(manager.registry.get("test.example"), "test.enabled")).toBe(
      true,
    );
    ...
    await expect(
      manager.setConfiguration("test.example", { "test.enabled": "wrong" }),
    ).rejects.toThrow();
  });
  test("dependencies enforce versions and cycles without hanging", async () => {
    const { manager } = setup();
    const a = other("test.a"),
      b = other("test.b");
    a.manifest.activationEvents = [];
    b.manifest.activationEvents = [];
    a.manifest.dependencies = { "test.b": "^1.0.0" };
    b.manifest.dependencies = { "test.a": "^1.0.0" };
    await manager.installExtension(a);
    await manager.installExtension(b);
    await manager.enableExtension("test.a", []);
...
test("command registry rejects duplicate handlers and disposes idempotently", async () => {
  ...
  const reservation = commands.reserve(archive().manifest);
  const disposable = commands.register("test.example", "test.hello", async () => 42);
  expect(() => commands.register("test.example", "test.hello", async () => 43)).toThrow(
    ExtensionError,
  );
  expect(await commands.execute("test.hello")).toBe(42);
  disposable.dispose();
  disposable.dispose();
  await expect(commands.execute("test.hello")).rejects.toThrow();
  reservation.dispose();
  expect(commands.list()).toEqual([]);
});
test("event bus isolates listener failures and snapshots payloads", async () => {
  const errors: unknown[] = [];
  const bus = new EventBus<{ event: { value: number } }>((error) => errors.push(error));
  const seen: number[] = [];
  bus.on("event", (event) => {
    event.value = 99;
    throw new Error("bad listener");
  });
  const listener = bus.on("event", (event) => {
...
test("disabling a dependency stops its active dependents", async () => {
  const { manager, runtime } = setup();
  const dependency = other("test.dependency"),
    dependent = other("test.dependent");
  dependent.manifest.dependencies = { "test.dependency": "^1.0.0" };
  await manager.installExtension(dependency);
  await manager.installExtension(dependent);
  await manager.enableExtension("test.dependency", []);
  await manager.enableExtension("test.dependent", []);
...
test("a new manager discovers persisted enablement and activates on startup", async () => {
  const { manager, storage } = setup();
  await manager.installExtension(other("test.persisted"));
  await manager.enableExtension("test.persisted", []);
  const runtime = new TestRuntime();
  const restored = new ExtensionManager(storage, runtime, fakeCore().core, "0.1.0");
  await restored.discover();
  await restored.trigger("onStartup");
...
function richArchive(): ExtensionArchive {
  return validateArchive({
    ...
    manifest: {
      ...
      engines: { l8db: ">=0.1.0 <1.0.0" },
      main: "dist/extension.js",
      activationEvents: ["onStartup", "onView:test.view"],
      permissions: [
        "database:read",
        "database:write",
        "network",
      ...
      capabilities: {
        network: { hosts: ["example.com", "*.example.org"] },
        process: { commands: ["tool"] },
      },
      contributes: {
        commands: [{ id: "test.run", title: "Run" }],
        configuration: { "test.mode": { type: "string", default: "a", enum: ["a", "b"] } },
        views: [{ id: "test.view", title: "View", location: "sidebar" }],
        panels: [{ id: "test.panel", title: "Panel" }],
        statusBar: [{ id: "test.status", alignment: "right", priority: 5 }],
        menus: [{ command: "test.run", location: "palette" }],
      },
    },
    files: { "dist/extension.js": "exports.activate = () => {}" },
  });
...
test("manifest validates views, panels, status bar, menus and capabilities", () => {
  ...
  expect(() =>
    validateManifest({
      ...base,
      contributes: { ...base.contributes, menus: [{ command: "missing", location: "palette" }] },
    }),
  ).toThrow();
  expect(() =>
    validateManifest({ ...base, capabilities: { network: { hosts: ["not a host!!"] } } }),
  ).toThrow();
  expect(() =>
    validateManifest({ ...base, capabilities: { process: { commands: ["../evil"] } } }),
  ).toThrow();
  expect(() =>
    validateManifest({
      ...base,
      contributes: { ...base.contributes, views: [{ id: "x", title: "X", location: "nowhere" }] },
    }),
  ).toThrow();
  expect(() =>
    validateManifest({ ...base, engines: { l8db: ">=0.1.0 <1.0.0", api: "^1.1.0" } }),
  ).not.toThrow();
  expect(() =>
    validateManifest({ ...base, engines: { l8db: ">=0.1.0 <1.0.0", api: "^2.0.0" } }),
  ).toThrow();
});
...
test("database.query enforces read and write permissions", async () => {
  ...
  expect(await denied("database.query", ["SELECT 1"])).toEqual({
    columns: ["n"],
    rows: [{ n: "1" }],
    rowsAffected: null,
    executionTimeMs: 1,
...
test("network.fetch enforces the host allowlist", async () => {
  ...
  expect(await rpc("network.fetch", ["https://example.com/api"])).toEqual({
    status: 200,
    headers: {},
    body: "ok",
  });
...
test("secrets, clipboard, files and processes are permission gated", async () => {
  ...
  await expect(allowed("workspace.readFile", ["/tmp/opened.txt"])).rejects.toThrow("dialog grant");
  ...
  expect(await allowed("process.run", ["tool", { args: ["--help"] }])).toEqual({
    status: 0,
    stdout: "out",
    stderr: "",
  });
...
test("views, status bar and panels push UI state", async () => {
  ...
  await manager.activate("test.rich");
  const rpc = runtime.rpc.get("test.rich")!;
  await rpc("views.setTree", [
    "test.view",
    [{ id: "a", label: "A", children: [{ id: "b", label: "B", command: "test.run" }] }],
  ]);
  expect(manager.listViews()[0].items).toHaveLength(1);
  await expect(rpc("views.setTree", ["unknown", []])).rejects.toThrow(ExtensionError);
  await expect(rpc("views.setTree", ["test.view", [{ id: "!!", label: "x" }]])).rejects.toThrow(
    ExtensionError,
  );
...
test("configuration enforces enums and notifies active extensions", async () => {
  ...
  await manager.enableExtension("test.rich", []);
  await manager.activate("test.rich");
  await expect(manager.setConfiguration("test.rich", { "test.mode": "c" })).rejects.toThrow(
    ExtensionError,
  );
