...
export function validateManifest(value: unknown): ExtensionManifest {
  ...
  if (value.contributes !== undefined) {
    ...
    if (panels !== undefined && (!Array.isArray(panels) || panels.length > 20 || !panels.every((p) => object(p) && string(p.id) && commandPattern.test(p.id) && string(p.title) && (p.surface === undefined || p.surface === "tab" || p.surface === "overlay")))) fail("Invalid panels");
    ...
    const menuLocations = ["palette", "view/title", "view/item", "statusBar", "toolbar"];
